...
commandList = ['uname', 'uptime -p', "top -bn1 | awk '/%Cpu/ {print $2}'","free -m | awk '/Mem:/ {print $3 / $2 * 100}'","df -h --output='avail' / | awk 'NR==2'" ,"ps aux | wc -l", "ps -eo nlwp | tail -n +2 | awk '{ num_threads += $1 } END { print num_threads }'","hostname -I | awk '{print $1}'"] #List of commands to execute
global resultDict
